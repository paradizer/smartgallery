jQuery(document).ready(function() {
    ...
    var firefox = [[0, 0], [1, 7], [2,10], [3, 8], [4, 10], [6, 8], [7, 8]];

    var chrome = [[0, 9], [1, 5], [2,8], [3, 6], [4, 8], [5, 3], [6, 6]];
    ...
    $.getJSON ("/amazon/get_aws_inf")
        .done(function (json) {

        var plot2 = jQuery.plot(jQuery("#basicflot2"),
            [ { data: json['aws_total_prev'],
         ...
         jQuery("#basicflot2").bind("plotclick", function (event, pos, item) {
            if (item) {
              plot2.highlight(item.series, item.datapoint);
                ...
                console.log("JSON1 Data: " + json['res1']);
        })
        .fail(function (bardata, textStatus, error) {
    ...
    var bardata = [ ["Mar", 18], ["Apr", 13], ["May", 17], ["Jun", 30], ["Jul", 26], ["Aug", 16], ["Sep", 17], ["Oct", 5], ["Nov", 8] ];
    ...
    var options1 = {
        ...
        yaxis: {
		  ...
		  xaxis: {
				mode: "categories",
				tickLength: 0,
                axisLabel: "Day before today"
		  }
	};
    $.getJSON ("/amazon/get_aws_inf")
        .done(function (json) {
           jQuery.plot('#barchart', [ json['aws_by_day'] ], options1);
            console.log("1JSON Data: " + json);
        })
        .fail(function (bardata, textStatus, error) {
            var err = textStatus + ", " + error;
            console.log("Request Failed: " + err);
